...
struct ClapPluginExtensions
{
  ...
  // AudioAlign patch: the AUv2 wrapper needs this to advertise every channel layout the
  // plugin supports, not just the one currently selected. See deps/PATCHES.md.
  const clap_plugin_audio_ports_config_t *_audioports_config = nullptr;
