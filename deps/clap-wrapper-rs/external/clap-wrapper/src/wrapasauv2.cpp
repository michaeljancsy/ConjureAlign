...
OSStatus WrapAsAUV2::Initialize()
{
  ...
  // AudioAlign patch: make sure the selected audio-ports-config matches the formats the
  // host actually negotiated before the plugin is activated. ChangeStreamFormat() normally
  // has done this already; this covers hosts that set formats by another route. select() is
  // only legal while deactivated, so it has to happen before activateCLAP().
  selectAudioPortsConfigForMain(kAudioUnitScope_Output,
...
// AudioAlign patch (see deps/PATCHES.md): clap-wrapper derives the AU channel layout from
// whichever CLAP audio-ports-config happens to be selected, and never calls
// `audio-ports-config::select`. That pins the AU to the plugin's first layout — for a
// plugin offering Stereo and Mono, the AU is stereo-only and hosts hide it on mono tracks.
// These three helpers let the AU advertise every config, accept a format belonging to any
// of them, and switch the CLAP plugin over once the host commits to one.

namespace
{
// Calls `fn(cfg)` for every audio-ports-config the plugin publishes, stopping early if it
// returns true. Returns whether any invocation returned true.
template <typename Fn>
bool forEachAudioPortsConfig(const Clap::Plugin *plugin, Fn &&fn)
{
  if (!plugin || !plugin->_ext._audioports_config) return false;
  auto apc = plugin->_ext._audioports_config;
  auto pl = plugin->_plugin;
  if (!apc->count || !apc->get) return false;
  ...
  for (uint32_t i = 0; i < n; ++i)
  {
    ...
    if (fn(cfg)) return true;
  }
  return false;
}

// The main-bus channel count this config presents on the given AU scope, or -1 if it has
// no main port there.
int mainChannelsFor(const clap_audio_ports_config_t &cfg, AudioUnitScope inScope)
{
  if (inScope == kAudioUnitScope_Input)
    return cfg.has_main_input ? (int)cfg.main_input_channel_count : -1;
...
bool WrapAsAUV2::selectAudioPortsConfigForMain(AudioUnitScope inScope, uint32_t channels)
{
  ...
  bool found = forEachAudioPortsConfig(
      _plugin.get(),
      [&](const clap_audio_ports_config_t &cfg)
      {
        if (mainChannelsFor(cfg, inScope) != (int)channels) return false;
        target = cfg.id;
        return true;
      });
                             ...
                             const AudioStreamBasicDescription &inNewFormat)
{
  ...
  // AudioAlign patch: the checks above only see the currently selected config. Also accept
  // a main-bus format that any other config supports — ChangeStreamFormat() selects it once
  // the host commits.
  if (inElement == 0 &&
      forEachAudioPortsConfig(_plugin.get(),
                              [&](const clap_audio_ports_config_t &cfg) {
                                return mainChannelsFor(cfg, inScope) ==
                                       (int)inNewFormat.mChannelsPerFrame;
                              }))
  {
    return true;
  }
                                        ...
                                        const AudioStreamBasicDescription &inNewFormat)
{
  ...
  // AudioAlign patch: the host has committed to a main-bus width. Move the CLAP plugin onto
  // the audio-ports-config that matches, so its ports (including the sidechain's width)
  // agree with what the AU elements now carry.
  if (res == noErr && inElement == 0)
  {
    selectAudioPortsConfigForMain(inScope, inNewFormat.mChannelsPerFrame);
  }
...
{
  // AudioAlign patch: prefer the full set of audio-ports-configs, so every layout the
  // plugin supports is advertised to the host rather than only the selected one.
  if (cinfo.empty())
  {
    forEachAudioPortsConfig(_plugin.get(),
                            [&](const clap_audio_ports_config_t &cfg)
                            {
                              auto in = mainChannelsFor(cfg, kAudioUnitScope_Input);
                              auto out = mainChannelsFor(cfg, kAudioUnitScope_Output);
                              if (in < 0 && out < 0) return false;
                              AUChannelInfo entry{static_cast<SInt16>(in < 0 ? 0 : in),
                                                  static_cast<SInt16>(out < 0 ? 0 : out)};
                              for (const auto &c : cinfo)
                                if (c.inChannels == entry.inChannels &&
                                    c.outChannels == entry.outChannels)
                                  return false;
                              cinfo.push_back(entry);
