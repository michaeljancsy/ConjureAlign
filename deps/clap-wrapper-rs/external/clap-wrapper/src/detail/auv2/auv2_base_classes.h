                   ...
                   public os::IPlugObject
{
 ...
 public:
  ...
  // AudioAlign patch (see deps/PATCHES.md): switch the wrapped CLAP plugin to the
  // audio-ports-config whose main bus on `inScope` has `channels` channels, so the AU is
  // not pinned to the plugin's first layout. Returns true if a config was selected.
  bool selectAudioPortsConfigForMain(AudioUnitScope inScope, uint32_t channels);
